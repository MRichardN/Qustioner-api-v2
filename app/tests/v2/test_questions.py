...
class TestQuestion(BaseTest):
    ...
    def test_post_question_meetup_not_created(self):
        ...
        self.question1.update({'meetup_id': 5})

        res = self.client.post('/api/v2/question/', data=json.dumps(self.question1),
                               headers=self.headers)
        data = res.get_json()
    ...
    def test_post_question_missing_fields(self):
        ...
        res = self.client.post('/api/v2/questions', json=self.question1,
                               headers=self.headers)
        data = res.get_json()
