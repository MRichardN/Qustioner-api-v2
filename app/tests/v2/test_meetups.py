...
class TestMeetup(BaseTest):
    ...
    def test_create_meetup_with_missing_fields(self):
        ...
        res = self.client.post('/api/v2/meetups', json=self.meetup1,
                               headers=self.headers)
        data = res.get_json()
    ...
    def test_create_meetup_with_past_date(self):
        ...
        self.assertEqual(res.status_code, 400)
        self.assertEqual(data['status'], 400)
        self.assertEqual(data['message'], 'Invalid data. Please enter all required fields')
    ...
    def test_get_upcoming_meetups_notexists(self):
        ...
        self.meetup1.update({'happeningOn': '12/02/2019'})
